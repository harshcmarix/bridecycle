...
<div class="career-index box box-primary">
    <div class="box-body admin_list hotel_list dataTables_wrapper form-inline dt-bootstrap">
        ...
        <div class="filter-div " id="filter-div" style="display: none">
            ...
            <div class="row">
                <div class="col-md-12">
                    <?= $this->render('_search_new_shop_owner_customer', ['model' => $searchModel]) ?>
                </div>
            </div>
        ...
        $gridColumns = [
            ...
            [
                ...
                'urlCreator' => function ($action, $model, $key, $index) {
                    return Url::to(['user/new-shop-owner-customer-' . $action . '/', 'id' => $model->id]);
                }
            ],
        ];

        echo GridView::widget([
            'id' => 'new-shop-owner-customer-user-grid',
...
//                GridView::TEXT => ['label' => 'Export as TEXT', 'filename' => 'File_Name -' . date('d-M-Y')],
            ...
            'toolbar' => [
                ...
                [
                    'content' =>
                        Html::button('<i class="fa fa-plus-circle"> Add New Shop Owner </i>', [
                            'class' => 'btn btn-success',
                            'title' => 'Add New Customer',
                            'onclick' => "window.location.href = '" . Url::to(['user/new-shop-owner-customer-create']) . "';",
                        ]),
                    'options' => ['class' => 'btn-group mr-2']
                ],
                [
                    'content' =>
                        Html::button('<i class="fa fa-refresh"> Reset </i>', [
                            'class' => 'btn btn-basic',
                            'title' => 'Reset Filter',
                            'onclick' => "window.location.href = '" . Url::to(['user/index-new-shop-owner-customer']) . "';",
                        ]),
                    'options' => ['class' => 'btn-group mr-2']
                ],
                '{toggleData}',
            ...
            'itemLabelSingle' => 'new shop owner',
            'itemLabelPlural' => 'New Shop Owners',
        ]);
        ?>
    </div>
</div>
...
<script type="text/javascript">
    ...
    $('document').ready(function () {
        ...
        var filter_selector = '#new-shop-owner-customer-user-grid-filters input';
        var isInput = true;
        ...
        $("body").on('beforeFilter', "#new-shop-owner-customer-user-grid", function (event) {
            if (isInput) {
                return submit_form;
            }
        });

        $("body").on('afterFilter', "#new-shop-owner-customer-user-grid", function (event) {
            if (isInput) {
                submit_form = false;
            }
        });
            ...
            .on('keyup', filter_selector, function (e) {
                ...
                if ((keyCode >= 65 && keyCode <= 90) || (keyCode >= 48 && keyCode <= 57) || (keyCode >= 96 && keyCode <= 105) || (keyCode >= 186 && keyCode <= 192) || (keyCode >= 106 && keyCode <= 111) || (keyCode >= 219 && keyCode <= 222) || keyCode == 8 || keyCode == 32) {
                    if (submit_form === false) {
                        submit_form = true;
                        $("#new-shop-owner-customer-user-grid").yiiGridView("applyFilter");
                    }
                }
            })
        ...
        var select;
        var submit_form = false;
        var select_filter_selector = '#new-shop-owner-customer-user-grid-filters select';
        var isSelect = true;
