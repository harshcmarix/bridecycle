...
class BridecycleToSellerPaymentsSearch extends BridecycleToSellerPayments
{
    ...
    {
        return [
            [['id', 'order_id', 'order_item_id', 'product_id', 'seller_id', 'status'], 'integer'],
            [['amount'], 'number'],
    ...
    public function search($requestParams, $userId = null)
    {
        ...
        if (!empty($requestParams['fields'])) {
            ...
            $select = ['bridecycle_to_seller_payments.*',];
        }
        ...
        $query->orderBy(['bridecycle_to_seller_payments.created_at' => SORT_DESC]);
        $query->groupBy('bridecycle_to_seller_payments.id');
        ...
        $modelsData = $activeDataProvider->getModels();
        $activeDataProvider->setModels($modelsData);
        return $activeDataProvider;
    }
}
